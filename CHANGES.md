...
**Mechanical sync.** `sync.mjs` wrote 33 clean updates (`docs/guide/*`, and reference files across `just-bleat-it`, `interrogate`, `how`, `why`, `typescript-best-practices`, `architect`) plus the two new skills and the logo. 80 files carrying port-specific edits (the rebrand to `bleat`/`just-bleat-it`, the Claude-primitive substitutions, and independent prose changes since the fork) collided with upstream's density/mannered-prose pass (`e8d856f`) and punctuation pass (`d7cde2b`) and were flagged for manual merge. A three-way merge (base = substituted old upstream, ours = local, theirs = substituted new upstream) resolved 30 of those cleanly — the local edits and upstream's cuts touched disjoint text. The remaining 34 are genuine conflicts where the port's own content (new sections, restructured steps, added model-selection prose) overlaps the lines upstream rewrote; they stay on the manual-merge backlog for a follow-up editorial pass. They are, as of this sync: `unslop`, `reflect`, `just-bleat-it` (`SKILL.md` and 20 playbooks), `interrogate`, `setup-bleat`, `arena`, `how`, `why` (`SKILL.md` and `references/sources/databricks.md`), `swarm`, `show-me-your-work`, `no-comments`, `automate-me`, `typescript-best-practices`, `architect`, `principle-subtract-before-you-add`.
