...
**New skills.** Two principle leaves land: `principle-attack-the-premise` (take a census before writing a second fix that assumes the same failed premise) and `principle-test-behavior-not-implementation` (call code the way its users do; assert observed results, not internals). Both carry upstream's `disable-model-invocation: true`, converted to `user-invocable: false` per the port's own convention for `principle-*` leaves (see 0.9.15-era `CONTRIBUTING.md` rule); the invariant script's frontmatter check passes on both.

**Plugin logo.** `assets/logo.png` is vendored from upstream (`efa2a53`, shrunk under 512 KiB in `7314f72`). Upstream wires it into Cursor's `.cursor-plugin/plugin.json`, which the port excludes as Cursor-runtime-only; `plugins/bleat/.claude-plugin/plugin.json` gets its own `"logo": "assets/logo.png"` for parity.

**Mechanical sync.** `sync.mjs` wrote 33 clean updates (`docs/guide/*`, and reference files across `just-bleat-it`, `interrogate`, `how`, `why`, `typescript-best-practices`, `architect`) plus the two new skills and the logo. 80 files carrying port-specific edits (the rebrand to `bleat`/`just-bleat-it`, the Claude-primitive substitutions, and independent prose changes since the fork) collided with upstream's density pass (`e8d856f`) and punctuation pass (`d7cde2b`). A three-way merge (base = substituted old upstream, ours = local, theirs = substituted new upstream) resolved 30 of those, 15 were no-ops, and 35 files carrying 64 overlapping hunks were merged editorially. The per-hunk decision trail is committed at [`.audit/pstack-0-15-0-merge.tsv`](.audit/pstack-0-15-0-merge.tsv).

**What the editorial merge took, and what it refused.** Upstream shipped two separable things in this window. The density and punctuation pass (mid-sentence semicolons to periods, cut justification clauses, and the `For "do this"` trigger lists dropped from playbook headers now that `just-bleat-it/SKILL.md` carries them) is skill content, which `CONTRIBUTING.md` puts on upstream's side of the sync boundary, so the port takes all of it. Diverging on style would re-conflict the same lines every sync.

The second is a forge-neutrality migration (`23a56e2`) that adds **Origin**, Cursor's internal forge, alongside GitHub: `origin pr ...` commands, a "resolve the forge before the first PR operation" preamble, and base-branch chains replacing Graphite stacks. The port rejects it. bleat targets Claude Code against GitHub with `gh`, so Origin is a Cursor-runtime concern in the same class as `control-cli`, and a bleat user cannot run `origin pr`. `babysit`, `shipping`, `opening-a-pr`, `autopilot-full`, and `autopilot-stack` keep their `gh` and Graphite text. Forge-agnostic improvements that arrived inside that commit were kept: `babysit`'s "no base retarget, rebase, stack-wide submit, or force-push" phrasing, `shipping`'s sharpened `git patch-id` rule, `opening-a-pr`'s briefing-shaped `## Descriptions` rewrite with its 40-line squash-body limit, and the regression-lane and dual-sided perf gate in `autopilot-full` and `multi-phase-plan`.

Port-specific content upstream never had was preserved against its deletions: the operator-gated merge in both autopilots (`the operator clicks every merge`), `babysit`'s note that `/babysit` remains the standalone entry point, `orchestrate`'s `~/.claude/orchestrate/` store and the reasoning for it, and all 14 `**Platform note.**` paragraphs.

**Critique Mode retires.** Upstream deleted `how`'s Critique Mode in `e8d856f`, along with `references/critic-prompt.md` and `references/critique-rubric.md`. A sync only ever adds and updates, so both files survived as orphans here; they are deleted, and the now-unconsumed `how critics` role leaves `models.json`, which unstamps it from `how` and `setup-bleat`.

**Four defects the 1.1.0-era merge shipped, now fixed.** A full-tree denylist scan (not just the files this sync touched) found `control-cli` and `control-ui` in `plugins/bleat/README.md`, Cursor's `generalPurpose` subagent spelling, a "not shipped here" section telling bleat users to install `cursor-team-kit` for skills bleat actually vendors, and `~/CLAUDE.md importsbleat-models.mdc` in the setup guide, where the generic `.cursor/rules/` rule had mangled a filename into prose.

**Sync tooling.** Four changes, each closing a gap this sync exposed. `tools/sync.mjs` now runs the three-way merge itself rather than reporting every port-edited file for hand-merging. Its denylist scan covers merged files, not only written ones, which is how the README defects slipped through last time. It reports files upstream deleted instead of leaving orphans unmentioned. `tools/substitutions.json` gains a specific rule mapping the model override sheet to `.claude/bleat-models.md` ahead of the generic `.cursor/rules/` rule, plus denylist entries for `generalPurpose`, `cursor-team-kit`, and the mangled `CLAUDE.md imports` form so any other path that falls through fails loudly.
