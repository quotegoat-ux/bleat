...
- **The `make-bot-ui` skill** (new in pstack 0.14.x). Wires a UI to a Grok Bot over Cursor webhook routines; no Claude Code analog here.
- **The benny automation pack** (`automations/benny/`, new in pstack 0.14.x). Built on Cursor's automations runtime; excluded via the component's `exclude` list in `tools/upstream.json`.
