...
**bleat is my answer.** these are the same skills i use everyday to ship high quality code at Cursor. this turns cursor into a real engineering team. the goal is not to maximize loc, in fact it's the opposite. bleat helps you write less, but higher quality code.

**bleat gives you fearless parallelism.** when you can go deep on one agent and trust it to write good, verifiable code, you can truly parallelize with confidence. start multiple agents up with `just-bleat-it` and trust that they'll apply rigorous engineering principles to their work.

**cursor gives you the best of all worlds.** every frontier model has its strengths and weaknesses. use any model with bleat. in fact, many of my skills use multi-model workflows to take advantage of each model's unique strengths.
...
1. run [`/setup-bleat`](./skills/setup-bleat/SKILL.md) and choose which models you want.
2. use [`/just-bleat-it`](./skills/just-bleat-it/SKILL.md) whenever you're doing anything that requires rigor.
...
use [`/just-bleat-it`](./skills/just-bleat-it/SKILL.md) at the start of a task. it reads your request, picks from a set of playbooks, and runs the other skills as the steps need them.
...
```
/just-bleat-it i'm going to bed. land the stack even if ci flakes. i want everything merged by
morning.
```
...
| playbook | for |
|---|---|
| [investigation](./skills/just-bleat-it/playbooks/investigation.md) | a read-only question. how does x work, why was y built this way, are we sure. |
| [bug fix](./skills/just-bleat-it/playbooks/bug-fix.md) | reproduce a defect, root-cause it, and fix with runtime evidence. |
| [perf](./skills/just-bleat-it/playbooks/perf-issue.md) | trace a measured slowness and improve it against a baseline. |
| [hillclimb](./skills/just-bleat-it/playbooks/hillclimb.md) | sustained, scientific improvement of one metric against a target, looping hypotheses with before/after measurement and one commit per accepted win. |
| [runtime forensics](./skills/just-bleat-it/playbooks/runtime-forensics.md) | diagnose a live symptom (leak, idle-cpu spin, glitch) from instrumentation. |
| [trace forensics](./skills/just-bleat-it/playbooks/trace-forensics.md) | diagnose a captured profiling artifact (cpuprofile, trace, spindump, heap snapshot). |
| [feature](./skills/just-bleat-it/playbooks/feature.md) | new or changed behavior, built from a named data shape. |
| [refactoring](./skills/just-bleat-it/playbooks/refactoring.md) | a behavior-preserving change to structure or shape. |
| [prototype](./skills/just-bleat-it/playbooks/prototype.md) | a throwaway sketch to make a design or behavioral decision cheaply, or to settle an empirical fork by observing it. |
| [visual parity](./skills/just-bleat-it/playbooks/visual-parity.md) | pixel-exact ui equivalence between two implementations. |
...
[`/just-bleat-it`](./skills/just-bleat-it/SKILL.md) is also a sticky mode: once entered it stays on across turns, applying itself when a playbook matches or the task needs rigor and staying out of the way otherwise. opt out any time by saying so.

[`/just-bleat-it`](./skills/just-bleat-it/SKILL.md) works extremely well with cursor's `/loop` command. you can make cursor work for many hours without sacrificing rigor.

## skills

[`/just-bleat-it`](./skills/just-bleat-it/SKILL.md) runs most of these for you when a step needs them (`how`, `why`, `architect`, `arena`, `swarm`, `interrogate`, `unslop`, `no-comments`, `technical-writing`, `tdd`, and the principles). the table below is for when you want one directly:
...
| skill | use it when |
|---|---|
| [`/just-bleat-it`](./skills/just-bleat-it/SKILL.md) | default entry point for any non-trivial task. |
...
| [`/setup-bleat`](./skills/setup-bleat/SKILL.md) | you want to pick which models bleat uses per role. detects your models and writes a config rule. |
...
mostly i type [`/just-bleat-it`](./skills/just-bleat-it/SKILL.md) at the start of a task and let it route to a playbook. the other skills fire as the steps need them. a few i reach for directly.
...
```
bug fix:           /just-bleat-it this pr has a subtle bug where the scroll drifts every 750ms even
                   when idle. repro first, then fix and verify.
perf:              /just-bleat-it a big list takes a second or two to load even though we virtualize.
                   run a cpu trace and tell me why.
feature:           /just-bleat-it build a small feature behind a feature flag. verify it really works.
prototype:         /just-bleat-it build two prototypes of the markdown renderer so we can compare.
                   spawn an agent for each.
multi-phase:       /just-bleat-it open source these skills as a plugin. nothing internal leaks, work
                   in a temp dir, show me the dependency graph first.
overnight run:     /just-bleat-it i'm going to bed. land the stack even if ci flakes. i want
                   everything merged by morning.
babysit:           /just-bleat-it check on pr 123. anything outstanding?
visual parity:     /just-bleat-it the row spacing is too tall when this flag is on. the second image
                   is correct. repro and fix until it matches.
figure it out:     /just-bleat-it i'm stepping away. migrate every caller from the synchronous store
...
bleat also ships [Comment Sicko](./agents/comment-sicko.md), a read-only comment reviewer available as `subagent_type: "Comment Sicko"`. usually invoke it through [`/no-comments`](./skills/no-comments/SKILL.md), not directly.
...
cursor already has a great plan mode which works great with bleat. but personally, i don't believe in planning. the best spec is code. if you do want to make a plan, [`/just-bleat-it`](./skills/just-bleat-it/SKILL.md) covers it, but it's not a default. 

## make it yours

`just-bleat-it` is my style. you may not want exactly that.

type [`/automate-me`](./skills/automate-me/SKILL.md). it mines your recent transcripts, drafts a `<your-name>-mode` skill from how you've actually worked, and routes through bleat underneath. you keep bleat as the base and end up with your own routing skill alongside `just-bleat-it`.

models are configurable too. type [`/setup-bleat`](./skills/setup-bleat/SKILL.md). it detects the models you have access to and writes a small always-applied rule mapping each role (code, judgment, the review panels) to a model. every skill reads it and falls back to sensible defaults when the rule is absent, so you override only what you want.

## automations

upstream pstack ships a dormant benny automation pack (slack triage, then reproduce-and-fix with real ui evidence). it is built on Cursor's automations runtime, so this port excludes it: `tools/upstream.json` lists `automations/` in the component's exclude list. read it upstream if you want it: https://github.com/cursor/plugins/tree/main/pstack/automations/benny
