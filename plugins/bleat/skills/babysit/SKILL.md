...
**Platform note.** On Codex or another non-Claude runtime, the Claude tool names and Claude built-in skills named below (`loop`, `AskUserQuestion`) are Claude defaults. Resolve them via [`codex-tools.md`](../just-bleat-it/references/codex-tools.md).

Inside just-bleat-it, the **Babysit** playbook ([`../just-bleat-it/playbooks/babysit.md`](../just-bleat-it/playbooks/babysit.md)) supersedes this skill: it owns mode declaration, the merge frontier, stack safety, and the `watch-pr` watcher. This skill stays the standalone `/babysit` entry point for a single PR outside a just-bleat-it run.

## When to use

- There's an open PR and the user explicitly wants it kept green, and you are not already inside a just-bleat-it run (the playbook owns that case).
...
- `just-bleat-it` opens here after a PR is opened.
