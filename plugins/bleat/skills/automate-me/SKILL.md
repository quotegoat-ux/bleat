...
**Platform note.** On Codex or another non-Claude runtime, the Claude tool names, `claude-*` slugs, and Claude built-in skills named below (including `plugin-dev:skill-development`) are Claude defaults. Resolve them via [`codex-tools.md`](../just-bleat-it/references/codex-tools.md).
...
The **just-bleat-it** skill shows the shape. Read it for granularity. Don't copy its content; the user's rules are not the same as just-bleat-it's.
...
- The **just-bleat-it** skill: example of the output shape.
