...
it("scans stacks tier-major so an ubleat conflict outranks frontier CI", async () => {
  ...
  const ubleat = await readSnapshot({
    reader: fakeReader({ facts: { mergeable: "CONFLICTING" } }),
    context: context(11),
    pendingHistory: "omit",
    allowDraft: false,
  });
  const decision = selectTierMajorStackDecision([frontier, ubleat]);
...
it("attributes a stack wait to the PR whose checks are pending, not the bottom", async () => {
  ...
  const pendingUbleat = await readSnapshot({
    reader: fakeReader({
      fastPath: { kind: "checks", checks: [pendingCheck("ubleat-build")] },
    }),
    context: context(21),
    pendingHistory: "omit",
    allowDraft: false,
  });
  const decision = selectTierMajorStackDecision([readyBottom, pendingUbleat]);
  expect(decision).toMatchObject({
    kind: "waiting",
    frontier: { number: 21 },
    pending: [{ name: "ubleat-build" }],
  });
});
