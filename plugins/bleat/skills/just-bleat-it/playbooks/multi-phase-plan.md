...
6. Run `node <base directory>/scripts/check-plan.mjs <plan.md>` and fix every line it prints (the **encode-lessons-in-structure** principle skill).
...
- [ ] Open the PR ready, never draft, with `gh pr create` and no `--draft` flag, or with Graphite `gt` for a stack.
