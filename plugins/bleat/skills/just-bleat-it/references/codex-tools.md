# Codex tool mapping for bleat

bleat skills are written in Claude Code tool language (the `Skill` tool, the `Agent` tool, `AskUserQuestion`, `claude-*` model slugs). On Codex the skills are the same files; only the tool names resolve differently. Read this when a bleat skill names a Claude tool, a Claude built-in skill, or a `claude-*` model.

## Tool actions

| bleat / Claude action | Codex equivalent |
|------------------------|------------------|
...
just-bleat-it's Subagents section sets Claude-specific defaults (`subagent_type: "bleat-agent"`, `run_in_background: true`). On Codex:

- There is no `bleat-agent` subagent type. Route an ad-hoc subagent through just-bleat-it's style by dispatching a `spawn_agent` whose instructions tell it to read the `just-bleat-it` skill in full first.
...
## Claude built-in skills bleat references

Some triggers name skills that ship with Claude Code, not bleat. They do not exist on Codex. Substitute the behavior:

| Claude built-in named in bleat | On Codex |
|---------------------------------|----------|
